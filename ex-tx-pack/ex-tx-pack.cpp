// ex-tx-pack.cpp : Defines the entry point for the console application.
//
...
int main() {
	...
	tx.inputs = ins;
	tx.locktime = 0;
	rawtx_len = bbp_tx_size(&tx, (bbp_sighash_t)0);
	rawtx = (uint8_t*)malloc(rawtx_len);
	bbp_tx_serialize(&tx, rawtx, (bbp_sighash_t)0);
